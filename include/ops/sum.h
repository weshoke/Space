...
namespace space {
    namespace ops {
        namespace detail {
            ...
            struct BitSum {
                using Lhs =
                    brigand::transform<B1, brigand::bind<AddLhs, brigand::pin<B1>, brigand::_1>>;
                using Rhs =
                    brigand::transform<B2, brigand::bind<AddRhs, brigand::pin<B2>, brigand::_1>>;
            ...
            template <class Summand, class A, class B>
            constexpr auto BladeSum(Summand, const A& a, const B& b)
            {
                return Summand::Value(a, b);
            }

            template <template <class...> class SumList, class... Summands, class A, class B>
            constexpr auto BasisSum(SumList<Summands...>, const A& a, const B& b)
            {
                ...
                (void)std::initializer_list<int>{
                    ((res += BladeSum(Summands{}, a, b)), void(), 0)...};
                return res;
            }

            template <template <class...> class SumLists,
                      class... Lists,
                      class Algebra,
                      template <class...> class Multivector,
                      class BasisA,
                      class BasisB>
            constexpr auto MultivectorSum(SumLists<Lists...>,
        ...
        constexpr auto Add(const Multivector<Algebra, BasisA>& a,
                           const Multivector<Algebra, BasisB>& b)
        {
